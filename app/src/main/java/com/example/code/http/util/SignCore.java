...
public class SignCore {
     ...
     * @return 签名结果字符串
     */
    public static String buildRequestSign(Map<String, Object> sParaTemp, String key) {

        Map<String, String> sPara = new HashMap<>();

        for (Map.Entry<String, Object> entry : sParaTemp.entrySet()) {
            String keyMap = entry.getKey(); // name
            String valueMap = entry.getValue().toString(); // value
            sPara.put(keyMap, valueMap);
        }
